...
class fan():
	...
	def plot_radar_fov(cls, dmap_data: List[dict], scan_index: int=0, 
		all_gates: bool = False, parameter: str = 'v', hemisphere: str='north', 
		lowlat: int=50, cmap: str = None, groundscatter: bool = False, zmin: int = None, 
		zmax: int = None, colorbar: plt.colorbar = None, colorbar_label: str = ''):		
		"""
		...
		hemisphere: str
			North or south, depending on radar. Currently, pyDARN cannot work this out
			itself, so you should always give it
			Default: 'north'. Alternatives: 'south'. What more did you expect?   
		lowlat: int
		...
		for x in range(fan_shape[0]):
			...
			#Convert to AACGM
				geomag = np.array(aacgmv2.get_aacgm_coord(beam_corners_lats[x,y], 
						beam_corners_lons[x,y], 250, dtime))		
				beam_corners_aacgm_lats[x,y]=geomag[0]
				beam_corners_aacgm_lons[x,y]=geomag[1]	
		
		#Work out shift due in MLT
		mltshift=beam_corners_lons[x,y]-(aacgmv2.convert_mlt(beam_corners_lons[x,y],dtime)*15)
		...
		for gates in range(rangelimit-1):
			...
			#Index colour table correctly
				cmapindex=(scan[gates,beams]+abs(zmin))/(abs(zmin)+abs(zmax))
				if cmapindex < 0:
					cmapindex = 0
				if cmapindex > 1:
					cmapindex = 1
			...
			#Check for zero values (white) and groundscatter (gray)			
				...
				if groundscatter and\
				   grndsct[gates,beams] == 1:
						colour_rgba = 'gray'					
					
				theta=[thetas[gates,beams],thetas[gates+1,beams],
					thetas[gates+1,beams+1],thetas[gates,beams+1]]
				r=[rs[gates,beams],rs[gates+1,beams],rs[gates+1,beams+1],rs[gates,beams+1]]
				ax.fill(theta,r,color=colour_rgba)			

		#Plot FOV outline
		plt.polar(thetas[0:rangelimit,0], rs[0:rangelimit,0], color='black',linewidth=0.5)
		plt.polar(thetas[rangelimit-1,0:thetas.shape[1]-1], 
			rs[rangelimit-1,0:thetas.shape[1]-1], color='black',linewidth=0.5)
		plt.polar(thetas[0:rangelimit,thetas.shape[1]-2], rs[0:rangelimit,thetas.shape[1]-2], 
			color='black',linewidth=0.5)
		plt.polar(thetas[0,0:thetas.shape[1]-2], rs[0,0:thetas.shape[1]-2], 
			color='black',linewidth=0.5)
		...
		if not colorbar:
			if parameter == 'v':
				...
				locator = ticker.MaxNLocator(min_n_ticks=3,
					integer=True, nbins='auto')						
			ticks = locator.tick_values(vmin=zmin, vmax=zmax)
			cb = ax.figure.colorbar(ax=ax, extend='both', ticks=ticks)
