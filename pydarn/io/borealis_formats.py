...
class BorealisRawacf():
    ...
    single_element_types = {
        "borealis_git_hash" : np.unicode, # Identifies the version of Borealis that made this data.
        ...
        "experiment_name" : np.unicode, # Name of the experiment file.
        "experiment_comment" : np.unicode,  # Comment about the whole experiment
        "slice_comment" : np.unicode, # Additional text comment that describes the slice.
        "num_slices" : np.int64, # Number of slices in the experiment at this integration time.
        "station" : np.unicode, # Three letter radar identifier.
        ...
        "scan_start_marker" : bool, # Designates if the record is the first in a scan.
        ...
        "samples_data_type" : np.unicode # C data type of the samples such as complex float.
    }
    ...
    array_dtypes = {
        ...
        "correlation_descriptors" : np.unicode, # Denotes what each acf/xcf dimension represents.
...
class BorealisBfiq():
    ...
    array_dtypes = {
        ...
        "beam_azms" : np.float64, # A list of the beams azimuths for each beam in degrees.
        "noise_at_freq" : np.float64, # Noise at the receive frequency, should be an array (one value per sequence) (TODO units??) (TODO document FFT resolution bandwidth for this value, should be = output_sample rate?)
        "antenna_arrays_order" : np.unicode, # States what order the data is in. Describes the data layout.
        "data_descriptors" : np.unicode, # Denotes what each data dimension represents.
