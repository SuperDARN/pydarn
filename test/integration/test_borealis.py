...
This test suite is to test the integration for the following classes:
    ...
    BorealisRestructureUtilities (implemented within BorealisRead/BorealisWrite)
...
class IntegrationBorealis(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.write_antennas_iq_site_file = 'test_antennas_iq.antennas_iq.hdf5.site' 
        self.source_antennas_iq_array_file = borealis_array_antennas_iq_file
        self.write_antennas_iq_array_file = 'test_antennas_iq.antennas_iq.hdf5.array' 
    ...
    def test_read_write_site_rawacf(self):
        ...
        dm = pydarn.BorealisRead(self.source_rawacf_site_file, 'rawacf', 
                                 'site')
        records = dm.records
        writer = pydarn.BorealisWrite(self.write_rawacf_site_file, records, 'rawacf', 
                                      'site')
        ...
        dictionaries_are_same = self.check_dictionaries_are_same(records, new_records)
    ...
    def test_read_write_array_rawacf(self):
        ...
        dm = pydarn.BorealisRead(self.source_rawacf_array_file, 'rawacf', 
                                 'array')
        arrays = dm.arrays
        writer = pydarn.BorealisWrite(self.write_rawacf_array_file, arrays, 'rawacf', 
                                      'array')
        ...
        dictionaries_are_same = self.check_dictionaries_are_same(arrays, new_arrays)
        self.assertTrue(dictionaries_are_same)
    ...
    def test_read_write_site_bfiq(self):
        ...
        dm = pydarn.BorealisRead(self.source_bfiq_site_file, 'bfiq', 
                                 'site')
        records = dm.records
        writer = pydarn.BorealisWrite(self.write_bfiq_site_file, records, 'bfiq', 
                                      'site')
    ...
    def test_read_write_array_bfiq(self):
        ...
        dm = pydarn.BorealisRead(self.source_bfiq_array_file, 'bfiq', 
                                 'array')
        arrays = dm.arrays
        writer = pydarn.BorealisWrite(self.write_bfiq_array_file, arrays, 'bfiq', 
                                      'array')
    ...
    def test_read_write_site_antennas_iq(self):
        ...
        Checks:
            ...
            - records written and then read are the same as original
        """
        dm = pydarn.BorealisRead(self.source_antennas_iq_site_file, 'antennas_iq', 
                                 'site')
        records = dm.records
        writer = pydarn.BorealisWrite(self.write_antennas_iq_site_file, records, 'antennas_iq', 
                                      'site')
        self.assertTrue(os.path.isfile(self.write_antennas_iq_site_file))
        dm2 = pydarn.BorealisRead(self.write_antennas_iq_site_file, 'antennas_iq', 
                                 'site')
        new_records = dm2.records
        dictionaries_are_same = self.check_dictionaries_are_same(records, new_records)
        self.assertTrue(dictionaries_are_same)
    ...
    def test_read_write_array_antennas_iq(self):
        """
        Test reading and then writing array structured antennas_iq data to a file. 

        Checks:
            - arrays that pass the read from a file can then be written
            - arrays written and then read are the same as original
        """
        dm = pydarn.BorealisRead(self.source_antennas_iq_array_file, 'antennas_iq', 
                                 'array')
        arrays = dm.arrays
        writer = pydarn.BorealisWrite(self.write_antennas_iq_array_file, arrays, 'antennas_iq', 
                                      'array')
        self.assertTrue(os.path.isfile(self.write_antennas_iq_array_file))
        dm2 = pydarn.BorealisRead(self.write_antennas_iq_array_file, 'antennas_iq', 
                                 'array')
        new_arrays = dm2.arrays
        dictionaries_are_same = self.check_dictionaries_are_same(arrays, new_arrays)
        self.assertTrue(dictionaries_are_same)
    ...
    def test_read_array_write_site_antennas_iq(self):
        ...
        writer = pydarn.BorealisWrite(self.write_antennas_iq_site_file, dm.records, 'antennas_iq', 
                                      'site')
        del dm, writer
        gc.collect()
        self.assertTrue(os.path.isfile(self.write_antennas_iq_site_file))
...
class IntegrationBorealisSDARN(unittest.TestCase):
    ...
    def setUp(self):
        ...
        writer = pydarn.BorealisWrite(self.bfiq_file, 
        ...
        writer = pydarn.BorealisWrite(self.rawacf_file, 
                                      self.rawacf_array_data, 'rawacf', 'array') 

        rawacf_reader = pydarn.BorealisRead(self.rawacf_file, 'rawacf', 'array')
    ...
    def test_bfiq2darniqdat(self):
        ...
        for record_num, record in enumerate(array_converter.sdarn_dict):
            dictionaries_are_same = self.check_dictionaries_are_same(
                                            record,
                                            site_converter.sdarn_dict[record_num])
            self.assertTrue(dictionaries_are_same)
